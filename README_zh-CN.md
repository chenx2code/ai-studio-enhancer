...
*   **自定义导出选项:** 点击浏览器工具栏的插件图标，可以打开设置菜单，自由切换是否包含：
    *   模型的思考过程 (Thought process)
    *   对话原链接 (Original link)
    *   谷歌账号邮箱 (Account email)
...
*   **快速导航:** 自动识别对话中的用户提问，支持一键跳转：
    *   文本提示显示截断预览（50字符）。
    *   图片提问显示 `[Image]` 标识。
    *   精准识别多种常见文件类型（如 `[PDF File]`, `[Markdown File]`, `[Audio File]`, `[YouTube Video]`）。
    *   未知的通用附件则显示 `[Document]` 标识。
    *   点击任意目录项快速跳转到对应位置。
*   **视觉反馈:** 导航时提供高亮效果，清楚标示目标位置。
*   **实时同步:** 对话变化时自动更新目录内容。
*   **一键跳转到底部:** 提供悬浮的“跳转到底部”按钮。当您向上滚动查看历史记录时自动出现，点击即可瞬间回到对话的最底部（最新内容）。
*   **键盘导航:** 增加可访问性：
    *   用户按 Tab 键可以在目录项之间导航。
    *   当某个目录项获得焦点时，会有视觉指示。
...
*   **快速回溯:** 在输入框中按下 `上键 (ArrowUp)` 和 `下键 (ArrowDown)` 即可像在 Terminal 终端中一样，无缝翻阅您之前的提问历史。
*   **智能防误触设计:** 专为多行文本编辑设计，绝不干扰正常打字：
    *   只有当光标位于输入框的**绝对最前**位置（首个字符前）时，按下 `上键` 才会进入历史模式。
    *   进入历史模式后，您可以连续按上下键快速翻阅。
    *   一旦您按下左右方向键、敲击字母或用鼠标点击输入框，系统会立刻判定您要编辑文本，并自动退出历史模式，恢复正常的文本编辑。
*   **草稿保护:** 如果您输入了一半内容突然想翻看历史，您的输入会被自动存为“草稿”。只要一直按下键回到最新的一条，就能完美恢复您的草稿。

### 🔔 智能后台桌面通知

*   **生成完毕提醒:** 当 AI 模型完成较长时间的文本生成时，自动发送系统原生的桌面通知，让您可以放心切换窗口处理其他事务。
*   **智能防打扰设计:** 只有当您的页面失去焦点（如：切换了标签页、浏览器最小化、或在副屏操作其他软件）时才会触发通知。如果您正注视着当前页面，将绝对不会有通知弹窗打扰您。
*   **上下文标题感知:** 通知卡片会自动带上当前对话的标题，如果您同时开启了多个对话，也能一眼分清是哪个任务完成了。
*   **自由控制:** 可以在插件的设置菜单 (Popup) 中随时一键开启或关闭此功能。
...
**请注意：** 对话内容会自动实时保存到 Google 云盘，为确保本扩展能获取最新内容，您只需留意页面上方出现 **"Saved to Drive"** 的状态提示即可。(本扩展依赖于 AI Studio 自动保存时触发的内部数据流来捕获完整的对话历史。)
